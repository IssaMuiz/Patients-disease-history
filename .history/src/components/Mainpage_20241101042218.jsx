...
const Mainpage = ({ data }) => {
  return (
    <main
      className="fixed top-0 bottom-0 xl:left-[350px] xl:right-[350px] mt-24 rounded-2xl overflow-scroll overflow-x-hidden scrollbar mx-auto
    "
    >
      ...
      <div className="mt-5 bg-white p-3  rounded-2xl">
        ...
        <div>
          {data.map((item, index) => {
            const diagnosisName = item.diagnostic_list.map((item) => item.name);
