      ...
      await fetch("https://fedskillstest.coalitiontechnologies.workers.dev", {
        ...
        .then((data) => {
          ...
          if (user && user.diagnosis_history.length > 0) {
            const labels = user.diagnosis_history.map((item, index) => {
            ...
            const systosieValues = user.diagnosis_history.map((item, index) => {
              if (item === undefined || item === null) {
                console.error(`item at index ${index} is undefined`);
              } else {
            ...
            const DiastolicValues = user.diagnosis_history.map(
              (item, index) => {
                if (item === undefined || item === null) {
                  console.error(`item at index ${index} is undefined`);
                } else {
